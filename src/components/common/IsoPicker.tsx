...
const IosPicker: FC<PropType> = (props) => {
  const {
    currentValue,
    slideCount,
    perspective,
  ...
  const slides = Array.from({ length: slideCount }, (_, index) => {
    ...
    if (value <= 0) return value + slideCount;
    return value;
  });

  const updateScrollIndex = useCallback((emblaApi: EmblaCarouselType) => {
    const index = emblaApi.selectedScrollSnap();
    const value = slides[index];
    onChange(value);
  }, []);
  ...
  useEffect(() => {
    ...
    emblaApi.on('select', () => updateScrollIndex(emblaApi));
    emblaApi.on('init', () => updateScrollIndex(emblaApi));
