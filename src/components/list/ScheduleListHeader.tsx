  ...
  return (
    <>
      <header className="sticky top-0 z-10 flex flex-col bg-white">
        <div className="sticky top-0 z-20 flex justify-between bg-white px-page pb-2 pt-[2rem] web:px-[28px]">
        ...
        <div
          className={`sticky top-0 flex transform items-end justify-between px-page pb-1.5 transition-all duration-300 ease-linear web:px-[28px] web:pb-3 ${
            isEdit ? 'translate-y-0' : 'h-0 -translate-y-24 opacity-0'
          }`}
        >
          <div className="flex items-center gap-3.5">
            <CheckButton all checked={allChecked} onClick={onCheckToggle} />
            <span className="text-black-900 text-1.1 font-bold xs:text-0.85">
              전체 선택
            </span>
          </div>
          <DeleteButtons
